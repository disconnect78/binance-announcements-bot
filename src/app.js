...
import realFs, { promises as fs } from 'fs'
...
async function run () {
  ...
  while (true) {
    ...
    let tokensBoughtAll = input.toString() !== '' ? JSON.parse(input.toString()) : []
...
async function addTokenToList (tokenSymbol) {
  const tokensInput = await fs.readFile(TOKENS_FILE)
  const tokens = JSON.parse(tokensInput.toString())
  ...
  const input = await fs.readFile(TOKENS_BOUGHT_FILE)
  const tokensBoughtAll = input.toString() !== '' ? JSON.parse(input.toString()) : []
  let tokenBought = tokensBoughtAll.find((token) => token.token === tokenSymbol)
...
async function processToken (tokenSymbol) {
  ...
  const input = await fs.readFile(TOKENS_BOUGHT_FILE)
  let tokensBoughtAll = input.toString() !== '' ? JSON.parse(input.toString()) : []
  const tokenBought = tokensBoughtAll.find((token) => token.token === tokenSymbol)
